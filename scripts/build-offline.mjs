...
function buildStandaloneHtml({ localFirst, fileName }) {
  const bootstrap = `<script>(function(){globalThis.__IPAW_OFFLINE_API_BASE__=${JSON.stringify(offlineApiBaseUrl)};globalThis.__IPAW_OFFLINE_OT_PROXY_BASE__=${JSON.stringify(offlineOperatingTheatreProxyBase)};globalThis.__IPAW_LOCAL_FIRST__=${JSON.stringify(localFirst)};function d(s){var b=atob(s),u=new Uint8Array(b.length);for(var i=0;i<b.length;i++)u[i]=b.charCodeAt(i);return new TextDecoder().decode(u)}function run(){var c=document.createElement('style');c.textContent=d('${cssBase64}');document.head.appendChild(c);(0,eval)(d('${jsBase64}'))}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',run,{once:true})}else{run()}})();</script>`;
