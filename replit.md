...
- V2 (`ipawv2.html`) treats IndexedDB as the primary local database. Local changes remain usable without a network, enter the durable backup queue, and are sent to Cloud in the background when connectivity returns. Cloud restore is explicit/manual so it cannot silently overwrite local work.
