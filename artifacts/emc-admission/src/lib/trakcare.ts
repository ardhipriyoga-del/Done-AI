...
export async function syncTrakCare(): Promise<SyncResult> {
  ...
  const upsertEpisode = async (
    ...
    if (existing && existing.id != null) {
      ...
      await db.put('episodes', {
        ...
        status,
        dischargeDate,
        archivedAt: status === 'pulang' ? Date.now() : existing.archivedAt,
      });
    } else {
      // Buat episode baru
      const newEp: Episode = {
  ...
  for (const patient of allPatients) {
    ...
    if (patient.status === 'pulang_pending') continue;  // menunggu konfirmasi petugas
    ...
    try {
      ...
      // Upsert episode sebagai 'pulang' — baik pulang maupun pulang_pending,
      // pasien sudah dinyatakan pulang oleh TrakCare → masuk riwayat.
      await upsertEpisode(
        patient.noRM,
        patient.episodeNo,
        patient.namaPasien,
