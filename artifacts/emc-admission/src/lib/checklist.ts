...
function isFilled(master: ChecklistMaster, value: string | undefined): boolean {
...
export function getChecklistStatus(
  ...
  const completedCount = visible.filter(master => isFilled(master, answers[master.id])).length;
  ...
  const complete = required.every(master => isFilled(master, answers[master.id]));
  return {
