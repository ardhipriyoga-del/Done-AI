...
export const KELAS_TARIF_OPTIONS = [
  { kelas: 'Kelas III',         tarif: 'Class III'                  },
  { kelas: 'Kelas II',          tarif: 'Class II'                   },
  { kelas: 'Kelas I',           tarif: 'Class I'                    },
  { kelas: 'VIP',               tarif: 'Class Premium/Mini VIP/VIP' },
  { kelas: 'Suite',             tarif: 'Class Suite/VVIP/Super VIP' },
  { kelas: 'Presidential Suite',tarif: 'Presidential Suites'        },
];

export function mapKelasTarif(roomType: string): string {
  const norm = (roomType ?? '').trim();
  const found = KELAS_TARIF_OPTIONS.find(o =>
    o.kelas.toLowerCase() === norm.toLowerCase() ||
    norm.toLowerCase().includes(o.kelas.toLowerCase())
  );
  return found?.tarif ?? norm;
}
...
export function lookupHarga(
  ...
  const byKelas = items.filter(i => i.kelasTarif === kelasTarif);
