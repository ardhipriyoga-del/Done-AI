  ...
  if (master.tipe === 'yesno') {
    return (
      <select value={value} onChange={event => onChange(event.target.value)} className={common}>
        <option value="">Pilih jawaban</option><option value="Ya">Ya</option><option value="Tidak">Tidak</option>
      </select>
    );
  }
              ...
              <div>
                ...
                <div className="rounded-lg border divide-y">
                  {historyMasters.map(master => {
                    ...
                    const checked = master.tipe === 'checkbox'
                      ? ['true', 'ya', '1'].includes(answer.toLowerCase())
                      : Boolean(answer.trim());
                    return (
