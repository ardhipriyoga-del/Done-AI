...
import {
  ...
  Bell, Check, CheckCheck, BellRing, FileText, Settings, Pencil, Save, QrCode,
} from 'lucide-react';
...
const AUTO_KEYS = new Set([
  ...
  'penjamin', 'diagnosa', 'salam', 'tanggal', 'jam', 'nama_petugas', 'no_hp_penanggung_jawab',
...
No. Rekam Medis : ${p.noRM}
Penjamin : ${p.payor || '-'}
Hari Perawatan : Hari ke-${hariRawat}

Berdasarkan perhitungan hingga saat ini, estimasi total biaya perawatan sementara adalah sebesar Rp ${estimasi.toLocaleString('id-ID')}.
...
function NotifikasiBillingTab() {
  ...
  const openBillingQr = async (p: Patient, hariRawat: number, estimasi: number) => {
    ...
    const message = buildBillingMessage(p, hariRawat, estimasi);
    setQrLoading(true);
    try {
