...
import {
  parseCPExcel, lookupHarga, mapKelasTarif, isObatKategori,
  fmtRp, KELAS_TARIF_OPTIONS, calcAdmin, ADMIN_RATE, ADMIN_MAX, MATERAI_DEFAULT,
  type MatchStatus, type CPRow,
} from '../lib/estimasi';
...
export default function EstimasiPanel({ isOpen, patient, user, onClose, isInline = false }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    const activeTarif = tarifs.filter(t => t.status === 'aktif').sort((a, b) => b.createdAt - a.createdAt)[0];
    if (activeTarif) {
      const mItems = await db.getAllFromIndex('masterTarifItems', 'masterTarifId', activeTarif.id!);
    ...
    if (existing) {
      ...
      setKelasTarif(existing.kelasTarif);
      ...
      const defaultKelas = patient.roomType?.trim() || '';
      const defaultTarif = mapKelasTarif(defaultKelas);
  ...
  const handleKelasChange = (kelas: string) => {
    const tarif = KELAS_TARIF_OPTIONS.find(o => o.kelas === kelas)?.tarif ?? kelas;
    setKelasPerawatan(kelas);
    setKelasTarif(tarif);
    if (masterItems.length > 0) {
      ...
      const materai = masterItems.find(i => i.kelasTarif === tarif && i.orderItem.toLowerCase().includes('materai'));
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!kelasTarif) { toast.error('Pilih Kelas Perawatan terlebih dahulu'); return; }
      ...
      <div className="flex flex-col gap-3 bg-muted/40 border border-border rounded-lg p-3 mt-1">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-muted-foreground">Kelas Perawatan</label>
            <select
              value={kelasPerawatan}
              onChange={e => handleKelasChange(e.target.value)}
              className="h-9 px-3 border border-input rounded-md bg-background text-sm min-w-[140px]"
            >
              <option value="">— Pilih Kelas —</option>
              {KELAS_TARIF_OPTIONS.map(o => (
                <option key={o.kelas} value={o.kelas}>{o.kelas}</option>
              ))}
            </select>
          </div>
