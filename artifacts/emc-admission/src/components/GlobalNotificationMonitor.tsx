...
import { apiUrl, getApiBaseUrl, hasTrakCareProxy } from '@/lib/apiConfig';
...
import { fetchIGDData, getEndpoints, type RawIGDPatient } from '@/lib/trakcareClient';
      ...
      try {
        ...
        try {
          const db = await getDB();
           ...
           const activePatients = patients.filter(patient => patient.status === 'aktif');
           const activePending = (await db.getAll('pendings'))
             .filter(item => item.status !== 'selesai' && Boolean(findMatchingPatient(activePatients, item)));
